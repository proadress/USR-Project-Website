export interface Item {
    url: string
    title: string
    cardDiscription: string
    content: string
    price: number
    image: string
}


export const items: Item[][] =
    [[
        {
            url: "plan1/item1",
            title: "潛能無限",
            cardDiscription: "發掘您潛在的無限可能性",
            content: "這個計畫旨在通過一系列的培訓和實踐，幫助參與者發現自己的潛在潛能。我們提供專業的指導，讓您在各個方面都能夠取得更好的突破。這是一個讓您挖掘潛能、迎接挑戰的絕佳機會。準備好迎接新的自己了嗎？加入我們吧！",
            price: 100,
            image: "plan1/itemCard1.jpg",
        },
        {
            url: "plan1/item2",
            title: "潛海戰將",
            cardDiscription: "成為海洋的征服者，挑戰自己的極限，參與激動人心的海洋冒險，體驗前所未有的冒險之旅。",
            content: "「潛海戰將」計畫將帶領您進入一場刺激的海洋冒險，挑戰自己的極限，探索未知的深海奧秘。我們提供專業的訓練和裝備，讓您安心享受這段令人難忘的旅程。不論您是初學者還是經驗豐富者，都能在這裡找到屬於自己的海洋冒險之旅。一起加入我們，成為真正的海洋征服者！",
            price: 110,
            image: "plan1/itemCard2.jpg",
        },
        {
            url: "plan1/item3",
            title: "探索海洋",
            cardDiscription: "深入探索神秘的海洋世界，發現奇幻的生物與美麗的景觀，讓您的探險之心得到滿足。",
            content: "「探索海洋」計畫將帶您穿越神秘的海洋世界，發現那些隱藏在深海底部的奇幻生物和令人驚嘆的美景。我們提供安全的探險體驗，並由專業的導遊帶領，讓您深入了解海洋生態，同時享受令人難以置信的探險之旅。準備好揭開海洋的神秘面紗了嗎？快來參加我們的「探索海洋」計畫吧！",
            price: 110,
            image: "plan1/itemCard3.jpg",

        },
        {
            url: "plan1/item4",
            title: "安全使用浮具遊憩推廣活動",
            cardDiscription: "學習如何安全、合理地使用浮具，參與推廣活動，享受輕鬆愉悅的水上遊憩時光。",
            content: "「安全使用浮具遊憩推廣活動」旨在教授參與者如何安全、合理地使用各種浮具，並鼓勵他們參與水上遊憩推廣活動。我們提供專業的教育和指導，讓您學習到正確的使用技巧，同時在輕鬆愉悅的氛圍中度過美好時光。無論您是初學者還是有一定經驗的愛好者，都能在這裡找到樂趣和滿足。歡迎加入我們的活動，一起享受水上遊憩的樂趣！",
            price: 110,
            image: "plan1/itemCard4.jpg",

        },
        {
            url: "plan1/item5",

            title: "2023清華大學獨木舟及SUP立式划槳體驗營",
            cardDiscription: "2023清華大學獨木舟及SUP立式划槳體驗營",
            content: "時間:7/17~7/21、8/7~8/11上午及下午共20場 每場次支援四位助教 清華大學成功湖 協助活動內容:水中安全認知與自救、器材裝備介紹、救生衣穿戴、獨木舟&SUP基本動作教學與操作",
            price: 110,
            image: "plan1/itemCard5.jpg",

        }], [],
    [
        {
            url: "plan3/item1",
            title: "商城",
            cardDiscription: "擺放子計畫提供商品及票劵，使用者透過網路下單購買",
            content: "擺放子計畫提供商品及票劵，使用者透過網路下單購買",
            price: 100,
            image: "plan3/item1.png",
        },
        {
            url: "plan3/item2",
            title: "博物館",
            cardDiscription: "結合個子計畫，將人文歷史生態保育等介紹展示。",
            content: "結合個子計畫，將人文歷史生態保育等介紹展示。",
            price: 110,
            image: "plan3/item2.png",
        },
        {
            url: "plan3/item3",
            title: "獨木舟體驗",
            cardDiscription: "讓使用者透過VR裝置及電腦設備，體驗獨木舟活動",
            content: "讓使用者透過VR裝置及電腦設備，體驗獨木舟活動",
            price: 110,
            image: "plan3/item3.png",

        },
        {
            url: "plan3/item4",
            title: "後端登入",
            cardDiscription: "提供FB、LINE、GOOGLE第三方登入，保障登入安全性",
            content: "提供FB、LINE、GOOGLE第三方登入，保障登入安全性",
            price: 110,
            image: "plan3/item4.png",

        },
    ], [], []
    ]